...
// src/lib/overview.cjs
var require_overview = __commonJS({
  "src/lib/overview.cjs"(exports2, module2) {
    "use strict";
    var { existsSync: existsSync2, readdirSync: readdirSync2 } = require("node:fs");
    var { join: join2 } = require("node:path");
    var { readWorkspaceStatus: readWorkspaceStatus2, pidAlive: pidAlive2 } = require_state_files();
    var LANE_STATUSES = {
      backlog: ["pending"],
      building: ["building"],
    ...
    function aggregateWorkspaces(entries, now = Date.now()) {
      ...
      for (const entry of entries) {
        ...
        const tickAgeSec = snapshot.lastTickTs ? (now - Date.parse(snapshot.lastTickTs)) / 1e3 : Infinity;
        const pendingWork = (snapshot.backlogCounts?.pending ?? 0) + (snapshot.backlogCounts?.building ?? 0) + (snapshot.backlogCounts?.verifying ?? 0);
    ...
    function readOverview2(root, { heartbeats = {}, now = Date.now(), readStatus = readWorkspaceStatus2 } = {}) {
      if (!existsSync2(root)) return aggregateWorkspaces([], now);
      const wsDirs = readdirSync2(root, { withFileTypes: true }).filter((d) => d.isDirectory() && existsSync2(join2(root, d.name, "config.yaml"))).map((d) => ({ name: d.name, path: join2(root, d.name) }));
      const entries = wsDirs.map(({ name, path }) => {
        try {
          const snapshot = readStatus(path);
          const running = heartbeats[path] != null && pidAlive2(heartbeats[path]);
    ...
    module2.exports = { aggregateWorkspaces, readOverview: readOverview2 };
  }
});
...
// src/lib/workspace.cjs
var require_workspace = __commonJS({
  "src/lib/workspace.cjs"(exports2, module2) {
    "use strict";
    var { existsSync: existsSync2, mkdirSync: mkdirSync2, writeFileSync: writeFileSync2, symlinkSync } = require("node:fs");
    var { join: join2 } = require("node:path");
    ...
    async function syncRepos2(wsPath, config, runGit) {
      ...
      for (const r of config.repos ?? []) {
        ...
        clones.push({
          name: r.name,
          ok: res.code === 0,
          error: res.code === 0 ? void 0 : (res.stderr || "git clone failed").trim().slice(-500)
        });
      }
      return clones;
    }
    ...
    async function scaffoldWorkspace2({ root, name, config, seanceRepo, runGit }) {
      ...
      const wsPath = join2(root, name);
      if (existsSync2(wsPath)) throw new Error(`workspace already exists: ${wsPath}`);
      const skillsSrc = join2(seanceRepo, "skills");
      if (!existsSync2(skillsSrc)) {
        throw new Error(`skills not found at ${skillsSrc} \u2014 set the seanceRepoPath plugin setting to your s\xE9ance checkout`);
      }
...
function activate(ctx) {
  ...
  ctx.ipc.handle("overview", () => readOverview(SEANCE_ROOT, { heartbeats: readHeartbeats(ctx), now: Date.now() }));
